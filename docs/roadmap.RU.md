...
- **Локальная реализация завершена (служебная идентичность БД, 2026-09-16)**:
  ...
  Чтение кэша через другое имя образа сравнивает неизменяемые digest и использует
  сохранённый путь снимка. Регрессия и полные Windows/Linux-наборы с настоящим
  PostgreSQL проходят; свежее общее покрытие prepare вне sandbox — 95,26%.
  Исправления ревью обеспечивают очистку после отмены, повтор неопубликованного
  снимка после retirement владельца и отдельные блокировки доступа экземпляров,
  чтобы долгие команды не блокировали другие БД. Регрессия и повтор на настоящем
  PostgreSQL проходят; текущее покрытие access — 95,6%, prepare — 95,3%.
...
- **Сделано (baseline M2 provenance + cache explain)**: single-stage local
  `plan` / `prepare` теперь поддерживают JSON side artifacts через
  `--provenance-path` для raw, alias-backed и bounded `--ref` flow; CLI также
...
- **Сделано (baseline M2 standalone `run --ref`)**: standalone local
  ...
  projected-cwd ref context и inputset semantics; на CLI boundary также
  режется ref-backed `prepare ... run`, пока не будет принят отдельный
  composite design.
...
- Project-tracked workflow aliases:
  ...
  - явное cwd-relative alias-ref resolution для `plan`, `prepare` и `run`
...
- Git-aware CLI (passive):
  - `diff` в path mode
  - bounded local `--ref` для `plan` / `prepare` / standalone `run` плюс
    ref-aware semantics в `diff` (`worktree|blob`)

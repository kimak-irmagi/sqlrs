...
4. `e2e_happy_path` downloads RC artifacts and runs release-gated matrix in
   ...
   - Windows: `hp-psql-chinook` with `btrfs` (host `sqlrs.exe` + WSL runtime).
     Native `copy` remains covered by platform tests; it is excluded from the
     hosted Windows release gate because that runner cannot provide a reachable
     Linux Docker image runtime to the native engine.
...
- Happy-path release matrix is blocking with platform axis:
  ...
  - Windows: scenario `hp-psql-chinook`; backend `btrfs` (`btrfs` uses the
    WSL-backed runtime; native `copy` is covered by platform tests).

...
4. `e2e_happy_path` скачивает RC-артефакты и выполняет release-gated матрицу в
   ...
   - Windows: `hp-psql-chinook` с `btrfs` (host `sqlrs.exe` + WSL runtime).
     Нативный `copy` остаётся покрыт платформенными тестами; из hosted Windows
     release gate он исключён, поскольку runner не предоставляет нативному
     engine доступный Linux Docker runtime.
...
- Happy-path release матрица является blocking с осью platform:
  ...
  - Windows: сценарий `hp-psql-chinook`; backend `btrfs`
    (`btrfs` использует WSL-backed runtime; нативный `copy` покрыт
    платформенными тестами).
